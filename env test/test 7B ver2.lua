-- 🍱 [야메추 모듈] RisuAI 공식 API 스펙 및 getFullChat 적용 최종 코드
...
function onInput(triggerId)
    ...
    -- getFullChat(triggerId)를 사용한 실제 유저/AI 대화 메시지 추출
    local userMsg = ""
    local aiMsg = ""
    ...
    if getFullChat then
        ...
        if type(chat) == "table" and #chat > 0 then
            -- 마지막 항목: 이번 유저 메시지
            userMsg = chat[#chat] and (chat[#chat].data or chat[#chat].message or "") or ""
            -- 직전 항목: 이전 AI 메시지
            aiMsg = (#chat >= 2) and chat[#chat - 1] and (chat[#chat - 1].data or chat[#chat - 1].message or "") or ""
        end
    end

    -- setState/getState로 메뉴 DB 복원
        ...
        if type(item) == "table" then
            ...
            if aiMsg ~= "" and string.find(aiMsg, name, 1, true) and 
               (string.find(userMsg, "싫") or string.find(userMsg, "별로") or string.find(userMsg, "다른") or string.find(userMsg, "안먹")) then
                ...
                log("🍱 거절 감지: " .. name .. " (누적: " .. tostring(item["refused_count"]) .. "회)")
            end
    ...
    -- 객체 상태 저장 (getState로 턴 간 지속)
    setState(triggerId, "menu_db", db)

    -- 프롬프트 노출용 채팅 변수 저장 ({{getvar::top_menu_json}} 매크로)
