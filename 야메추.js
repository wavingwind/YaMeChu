...
const 야메추 = (() => {
  // 1. DB 초기화
  function initMenuDB() {
    return Risuai.getVar("menu_db").then((currentDB) => {
      if (!currentDB) {
        const initialDB = {
        ...
        return Risuai.setVar("menu_db", JSON.stringify(initialDB)).then(() => {
  ...
  function calculateMenuScores() {
    return initMenuDB().then(() => {
      return Risuai.getVar("menu_db");
    }).then((dbRaw) => {
      let db = {};
      ...
      return Risuai.setVar("top_menu_json", JSON.stringify(top4, null, 2)).then(() => {
        console.log("🍱 [야메추] 점수 계산 완료 / Top 4:", top4);
      });
    });
  }

  // 3. 백엔드 대화 감지 및 DB 업데이트
  function onGenerateFinished(arg) {
    ...
    return Risuai.getVar("menu_db").then((dbRaw) => {
      ...
      if (updated) {
        return Risuai.setVar("menu_db", JSON.stringify(db)).then(() => {
  ...
  return {
    init: initMenuDB,
    run: calculateMenuScores,
    onGenerateFinished: onGenerateFinished
  };
})();

Risuai.registerPlugin(야메추);