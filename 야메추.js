...
const 야메추 = (() => {
  ...
  const initMenuDB = async () => {
    try {
      ...
      console.error("🍱 [야메추] DB 초기화 중 오류:", e);
    }
  };

  // 2. 점수 계산 메인 로직
  const calculateMenuScores = async () => {
    await initMenuDB();
  ...
  // 3. 백엔드 대화 감지 이벤트
  const onGenerateFinished = async (arg) => {
...
// RisuAI 이벤트 바인딩 (표준 API)
if (typeof Risuai !== "undefined" && Risuai.on) {
  Risuai.on("start", 야메추.calculateMenuScores);
  Risuai.on("generateFinished", 야메추.onGenerateFinished);
} else {
  // 모듈 바인딩 호환성 유지
  window.야메추 = 야메추;
}
